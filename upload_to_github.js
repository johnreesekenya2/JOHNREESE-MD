...
class GitHubUploader {
    ...
    async createRepository() {
        try {
            ...
            const repoData = {
                name: this.repoName,
                description: 'JOHNREESE-MD - Advanced WhatsApp Bot with Multi-Device Support, Anti-Delete, Media Downloads & More',
    ...
    async uploadProject() {
        try {
            console.log(chalk.cyan('🚀 Starting JOHNREESE-MD upload to GitHub...'));
            ...
            for (const file of files) {
                ...
                if (content) {
                    await this.uploadFile(
                        file.path, 
                        content, 
                        `Add ${file.path} - JOHNREESE-MD Bot`
                    );
                    successCount++;
                }
...
╚═══════════════════════════════════════╝
    ...
    const REPO_NAME = process.env.REPO_NAME || 'JOHNREESE-MD';
